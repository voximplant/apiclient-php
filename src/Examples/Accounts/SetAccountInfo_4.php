 ...
 * callback_url - If URL is specified, Voximplant cloud makes HTTP POST requests to it when something happens. For a full list of reasons see the type field of the [AccountCallback] structure. The HTTP request has a JSON-encoded body that conforms to the [AccountCallbacks] structure
 * callback_salt - If salt string is specified, each HTTP request made by the Voximplant cloud toward the callback_url has a salt field set to MD5 hash of account information and salt. That hash can be used be a developer to ensure that HTTP request is made by the Voximplant cloud
...
$params->location = 'GMT-8';
$params->min_balance_to_notify = 1.5;
$params->tariff_changing_notifications = true;
$params->news_notifications = true;
